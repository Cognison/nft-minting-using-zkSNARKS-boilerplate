...
contract Ownable {   
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != _owner, "Cannot transfer ownership to the original owner");
        require(newOwner != address(0), "Invalid address to transfer ownership to");
        
        address previousOwner = _owner;

        _owner = newOwner;

        emit TransferedOwnership(previousOwner, newOwner);
    }
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {        
        ...
        require(ownerOf(tokenId) != to, "Cannot approve for the owner");
        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true        
        require(msg.sender == _owner || isApprovedForAll(_tokenOwner[tokenId], msg.sender), "Caller must be owner of 'isApprovedForAll' must be true.");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        require(_tokenOwner[tokenId] != address(0) , "Token not found.");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "From address must be the current owner");
...
contract MyNFTERC721Token is ERC721Metadata {

    string private _name = "My NFT Token";

    string private _symbol = "MYT";

    string private _baseTokenURI = "https://<your-base-uri>/";
